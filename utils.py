...
from models.Ablation.FFCSWINT import FFCSWINT
from models.Ablation.FFCUNET import FFCUNET
from models.Ablation.SWINU import SWINU
from models.IDAT import IDAT
from models.IFastT import IFastT

sys.path.append('models/MSST-Net/')
...
from models.MSIGFusNet import MSIGFusNet
from models._3DT_Net import _3DT_Net
# from models.IFT_Net import IFT_Net
from models.DAFT import DAFT
from models.FastT import FastT


from models.SCANET import SCANet
from models.DuSFE import DuRegister_DuSE
import torch
import numpy as np
...
def args_parser():
    ...
    parser.add_argument('--arch', type=str, default='IFastT', help='SSFCNN ,FSwinU''IFastT'
                                                               ' MSSTNET ,IFT_Net'
                                                               'IDAT ,Fuformer'
                                                               'ResTFNet,3DT_Net'
                                                               ' MSDCNN ,SSRNET ,'
                                                               ...
                                                               'MSIGFusNet,DAFT,'
                                                               'FastT, SCANet'
                                                                'SWINU''FFCUNET''FFCSWINT'                )
    parser.add_argument('--dataset', type=str, default='CAVE', help='CAVE WDCM or PU')
    parser.add_argument('--upscale_factor', type=int, default=8, help="8 16 ")
    parser.add_argument('--batchSize', type=int, default=6, help='training batch size')
    parser.add_argument('--patch_size', type=int, default=8, help='cave: 64 32 wdcm and PU: 16 8 ')
...
def select_model(args,device):
    ...
    if args.arch == 'SSFCNN': #1
    ...
    elif args.arch == 'MSSTNET':#1
      MSSTarg = argsParser()
      model = MSST(MSSTarg).to(device)
    elif args.arch == 'IDAT': #1
      model = IDAT(args.hsi_chans,
                   args.upscale_factor,
                   args.patch_size).to(device)
    elif args.arch == 'IFastT':
      model = IFastT(args.hsi_chans,
                       args.upscale_factor,
                       args.patch_size).to(device)
      ...
      model = SpatCNN(args.upscale_factor,
                     args.msi_chans,
                     args.hsi_chans).to(device)
    elif args.arch == 'SpecCNN':
      model = SpecCNN(args.upscale_factor,
                     args.msi_chans,
                     args.hsi_chans).to(device)
    elif args.arch == 'MSIGFusNet':
      model = MSIGFusNet(args.upscale_factor,
                     args.msi_chans,
                     args.hsi_chans).to(device)
    elif args.arch == 'DAFT':
      model = DAFT(args.hsi_chans,
                      args.msi_chans,
                      args.upscale_factor).to(device)
    elif args.arch == 'FastT':
      model = FastT(args.hsi_chans,
                      args.msi_chans,
                      args.upscale_factor).to(device)
    elif args.arch == 'SCANet':
      model = SCANet(args.upscale_factor,
                       args.msi_chans,
                       args.hsi_chans).to(device)
    # elif args.arch == 'IFT_Net':
      # model = IFT_Net(args.hsi_chans,
      #                 args.msi_chans,
      #                 args.upscale_factor).to(device)
