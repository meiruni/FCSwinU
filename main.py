...
def test(path,save_path):
    load_dict = torch.load(path +'/'+ "{}.pth".format('epoch10WDCM'))
    opt.lr = load_dict['lr']
...
if __name__ == '__main__':
    ...
    tb_logger = SummaryWriter(log_dir='/root/tf-logs/' + opt.arch + '/' + current_time)
    current_step = 0

    # train
    if opt.mode == 1:
        # load_pretrained_dict(path,opt,model,optimizer)
        # load_dict = torch.load(path +'/'+ "{}.pth".format('epoch100WDCM'))
        # opt.lr = load_dict['lr']
        # epoch = load_dict['epoch']
        # model.load_state_dict(load_dict['param'])
        # optimizer.load_state_dict(load_dict['adam'])
